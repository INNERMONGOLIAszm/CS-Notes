...
class Solution {
    public int longestConsecutive(int[] nums) {
        ...
        for (int num : nums) {
            num_set.add(num);  //所有的数加进去去重
        }

        int longestStreak = 0;

        for (int num : num_set) {
            if (!num_set.contains(num - 1)) { //如果有正好比他小1的数，说明不应该从他开头计数。跳过
                int currentNum = num;  //如果找到了一个开头数
                int currentStreak = 1;  //初始化长度为1

                while (num_set.contains(currentNum + 1)) { //找比“头”大1的数，num加，长度也加
                    currentNum += 1;
                    currentStreak += 1;
                }

                longestStreak = Math.max(longestStreak, currentStreak); //和以前记录过的长度相比较
            }
        }
